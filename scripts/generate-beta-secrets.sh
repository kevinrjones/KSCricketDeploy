#!/usr/bin/env bash
# Generate private/beta secret files + optional Data Protection PFX for beta.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DIR="$ROOT/private/beta"
CERT_DIR="$ROOT/certs/beta"
ENV_FILE="$ROOT/environments/beta/.env"
...
if [[ ! -f "$PFX" || "${FORCE_PFX:-}" == "1" ]]; then
  ...
  openssl req -x509 -newkey rsa:2048 -nodes -days 825 \
    ...
    -subj "/CN=Identity data protection beta/O=KnowledgeSpike" >/dev/null 2>&1
  ...
  rm -rf "$tmp"
  chmod 644 "$PFX"
  printf 'write %s\n' "certs/beta/ids-mysql-dp.pfx"
else
  printf 'keep  certs/beta/ids-mysql-dp.pfx\n'
fi
